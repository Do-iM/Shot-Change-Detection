...
def generate(gen_f, set_size, dissolve_level = [10, 15], fade_level = [10, 15]):
    os.mkdir("0_Non")
    os.mkdir("1_Cut")
    os.mkdir("2_Dissolve")
    os.mkdir("3_Fade")
    ...
    for i in range(0, set_size):
        ...
        img_set = []
        for f_index in gen_f.index:
            img = Image.open('Base/' + basedir[dir_index] + '/' + imgs[img_index + f_index])
            img = img.resize(image_size)
            img_set.append(img)

        # merge imgs / save to 'Non/#.img'
        gen_f.gen(img_set).save('0_Non/' + str(i) + '.jpg')
    ...
    for i in range(0, set_size):
        ...
        for f_index in gen_f.index:
            if f_index < gen_f.change:
                img = Image.open('Base/' + basedir[dir_index[0]] + '/' + imgs_a[img_index_a + f_index])
            else:
                img = Image.open('Base/' + basedir[dir_index[1]] + '/' + imgs_b[img_index_b + f_index - gen_f.change])
            img = img.resize(image_size)
            img_set.append(img)

        # merge imgs / save to 'Cut/#.img'
        gen_f.gen(img_set).save('1_Cut/' + str(i) + '.jpg')
    ...
    for i in range(0, set_size):
        ...
        for f_index in gen_f.index:
            img_a = Image.open('Base/' + basedir[dir_index[0]] + '/' + imgs_a[img_index_a + f_index])
            img_a = img_a.resize(image_size)
            img_b = Image.open('Base/' + basedir[dir_index[1]] + '/' + imgs_b[img_index_b + f_index])
        ...
        gen_f.gen(img_set).save('2_Dissolve/' + str(i) + '.jpg')
    ...
    for i in range(0, set_size):
        ...
        for f_index in gen_f.index:
            img_a = Image.open('Base/' + basedir[dir_index] + '/' + imgs[img_index + f_index])
            img_a = img_a.resize(image_size)
            img_b = bw
            if bw_switch:
        ...
        gen_f.gen(img_set).save('3_Fade/' + str(i) + '.jpg')
